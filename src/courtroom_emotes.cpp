...
void Courtroom::initialize_emotes()
{
  ...
  ui_emote_left = new AOButton(options, m_resolver, this);
  ui_emote_left->setObjectName("ui_emote_left");
  ui_emote_right = new AOButton(options, m_resolver, this);
  ...
  emote_preview = new AOEmotePreview(ao_app, m_resolver, this);
  emote_preview->setObjectName("ui_emote_preview");
  emote_preview->resize(m_viewport_width, m_viewport_height);
...
void Courtroom::refresh_emotes()
{
  ...
  QString selected_image = m_resolver.currentThemeFilePath("emote_selected", kal::ImageAssetType).value_or(QString());
  for (int i = 0; i < max_emotes_on_page; ++i)
  {
    int x_pos = (button_width + x_spacing) * x_mod_count;
    int y_pos = (button_height + y_spacing) * y_mod_count;

    AOEmoteButton *f_emote = new AOEmoteButton(i, button_width, button_height, ao_app, m_resolver, ui_emotes);
...
void Courtroom::set_emote_dropdown()
{
  ...
  for (int n = 0; n < total_emotes; ++n)
  {
    ...
    QString icon_path = m_resolver.characterFilePath(current_char, "emotions/button" + QString::number(n + 1) + "_off", kal::AnimatedImageAssetType).value_or(QString());
...
void Courtroom::update_emote_preview()
{
  ...
  {
    preview_emote(pre, kal::PreEmote);
  }
  else
  {
    preview_emote(ao_app.get_emote(current_char, current_emote), kal::IdleEmote);
  }
}
...
void Courtroom::show_emote_menu(const QPoint &pos)
{
  ...
  {
    emote_menu->addAction("Preview pre: " + f_pre, this, [this, f_pre] { preview_emote(f_pre, kal::PreEmote); });
  }

  QString f_emote = ao_app.get_emote(current_char, emote_num);
  if (!f_emote.isEmpty())
  {
    emote_menu->addAction("Preview idle: " + f_emote, this, [this, f_emote] { preview_emote(f_emote, kal::IdleEmote); });
    emote_menu->addAction("Preview talk: " + f_emote, this, [this, f_emote] { preview_emote(f_emote, kal::TalkEmote); });

    bool post_emote_available = false;
    QList<kal::MaybePath> c_paths = {m_resolver.characterFilePath(current_char, "(c)" + f_emote, kal::AnimatedImageAssetType), m_resolver.characterFilePath(current_char, "(c)/" + f_emote, kal::AnimatedImageAssetType)};
    for (const auto &c_path : c_paths)
    {
      if (c_path)
      {
    ...
    if (post_emote_available)
    {
      emote_menu->addAction("Preview segway: " + f_emote, this, [this, f_emote] { preview_emote(f_emote, kal::PostEmote); });
    }
  }
  emote_menu->popup(button->mapToGlobal(pos));
}

void Courtroom::preview_emote(QString f_emote, kal::EmoteType emoteType)
{
  emote_preview->show();
  emote_preview->raise();
  emote_preview->updateViewportGeometry();
