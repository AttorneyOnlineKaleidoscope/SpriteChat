...
{
  ui_evidence = new AOImage(m_resolver, this);
  ...
  ui_evidence_left = new AOButton(options, m_resolver, ui_evidence);
  ui_evidence_left->setObjectName("ui_evidence_left");
  ui_evidence_right = new AOButton(options, m_resolver, ui_evidence);
  ui_evidence_right->setObjectName("ui_evidence_right");
  ui_evidence_present = new FlipButton(options, m_resolver, ui_evidence);
  ...
  ui_evidence_image_button = new AOButton(options, m_resolver, ui_evidence_overlay);
  ui_evidence_image_button->setText(tr("Choose.."));
  ui_evidence_image_button->setObjectName("ui_evidence_image_button");
  ui_evidence_x = new AOButton(options, m_resolver, ui_evidence_overlay);
  ...
  ui_evidence_ok = new AOButton(options, m_resolver, ui_evidence_overlay);
...
void Courtroom::refresh_evidence()
{
  ...
  ui_evidence_present->setImage("present_disabled", "present");
  ...
  for (int i = 0; i < max_evidence_on_page; ++i)
  {
    ...
    AOEvidenceButton *f_evidence = new AOEvidenceButton(i, button_width, button_height, options, m_resolver, ui_evidence_buttons);
...
void Courtroom::set_evidence_list(QList<kal::Evidence> &p_evi_list)
{
  ...
  QList<kal::Evidence> old_list = local_evidence_list;
...
void Courtroom::set_evidence_page()
{
  ...
  for (int n_evidence_button = 0; n_evidence_button < evidence_on_page; ++n_evidence_button)
  {
    ...
    {
      f_evidence_button->setEvidenceImage(local_evidence_list.at(n_real_evidence).image);
    ...
    {
      f_evidence_button->setEvidenceImage("");
    }
...
{
  QDir dir(kal::AssetPathResolver::applicationAssetFilePath("evidence"));
  ...
  bases.prepend(kal::AssetPathResolver::applicationAssetPath());
...
void Courtroom::on_evidence_clicked(int p_id)
{
  ...
  if (f_real_id == local_evidence_list.size())
  {
    if (current_evidence_global)
    {
      ao_app.createAndShipPacket<kal::CreateEvidencePacket>();
    }
    else
    {
      kal::Evidence f_evi;
...
void Courtroom::on_evidence_double_clicked(int p_id)
{
  ...
  kal::Evidence f_evi = local_evidence_list.at(f_real_id);
...
void Courtroom::on_evidence_delete_clicked()
{
  ...
  if (current_evidence_global)
  {
    ao_app.createAndShipPacket<kal::DeleteEvidencePacket>(current_evidence);
  }
  else
  {
...
void Courtroom::on_evidence_ok_clicked()
{
  ...
  if (current_evidence < local_evidence_list.size())
  {
    kal::Evidence f_evi = local_evidence_list.at(current_evidence);
    if (current_evidence_global)
    {
      ao_app.createAndShipPacket<kal::EditEvidencePacket>(current_evidence, f_evi);
    }
    else
    {
...
void Courtroom::on_evidence_transfer_clicked()
{
  ...
  if (!current_evidence_global) // Transfer private evidence to global
  {
    kal::Evidence f_evi = local_evidence_list.at(current_evidence);
    name = f_evi.name;
    ao_app.createAndShipPacket<kal::CreateEvidencePacket>(f_evi);
  }
...
void Courtroom::on_evidence_save_clicked()
{
  ...
  QDir dir(m_resolver.applicationAssetPath());
  if (!QFileInfo::exists(dir.absoluteFilePath("inventories")))
  {
    dir.mkdir("inventories");
  }
...
void Courtroom::evidence_load(QString filename)
{
  ...
  {
    kWarning() << "Trying to load a non-existant evidence save file:" << filename;
    return;
  }
...
bool Courtroom::compare_evidence_changed(kal::Evidence evi_a, kal::Evidence evi_b)
{
